    ...
    private function print_children($parent, $hierachy)
    {
    	...
    	if (isset($this->children[$parent]))
    	{
    		foreach($this->children[$parent] as $comment)
    		{
				...
				$output .= '<div id="comment-upvoter" data-comment-id="' . $comment->hashid() . '">';
				$output .= '<i class="ion-arrow-up-a ' . ($comment->isUpvoted() ? 'activated' : '') . ' " id="comment-upvoter-' . $comment->hashid() . '"></i>';
				$output .= '</div>';
				$output .= '<div id="comment-downvoter" data-comment-id="' . $comment->hashid() . '">';
				$output .= '<i class="ion-arrow-down-a ' . ($comment->isDownvoted() ? 'activated' : '') . ' " id="comment-downvoter-' . $comment->hashid() . '"></i>';
				$output .= '</div>';
				$output .= '</div>';
				$output .= '</div>';
				...
				$output .= '&middot;';
				$output .= '&nbsp;';
				$output .= '<span id="voteCountComment-' . $comment->hashid() . '">' . $comment->totalVotes() . '</span> point' . ($comment->totalVotes() == 1 ? '' : 's');
				$output .= '</p>';
				$output .= '</div>';
				$output .= '</header>';
				...
				$output .= '</section>';
				$output .= '<footer>';
				$output .= '<a id="replyToComment" data-id="' . $comment->hashid() . '">reply</a>';
				$output .= '<a>&middot;</a>';
				$output .= '<a href="' . $comment->getPermalink() . '">permalink</a>';
				$output .= '</footer>';
				$output .= '<div class="comment-box" id="commentBox-' . $comment->hashid() . '">';
